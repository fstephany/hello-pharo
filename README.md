...
## Server Directory Structure
    ...
    │       ├── app.pid
    │       ├── base-repo
    │       ├── current -> /home/azureuser/apps/hello-pharo/releases/2014-08-12.150722
    │       └── releases
    │           ├── 2014-07-16.135727
    │           │   ├── ....
    │           ├── 2014-07-16.141804
    ...
    └── stock-images
        ...
        ├── Pharo-30852.image
        ├── PharoV30.sources
        └── PharoV30.sources.zip


This directory structure is the one that will be generated by the current HelloPharo
installation.

### `~/stock-images`

This is the directory where the clean images are stored. Everytime you deploy,
...
- The pid file of the version of the app that is currently running.
- a `base-repo` directory into which your code will be checkout from the
  git repository
- a `current` symlink that points to the current version of the app.
- a `releases` directory that contains **all** the releases of your app.
  releases are identified with a timestamp.
...
1. A new directory is created in the `releases` folder
2. The code is checked out from the git repository
3. The code is copied in to the newly created directory
4. A clean image from `stock-images`is copied in the directory
5. The `install.st` script is run against the clean image
6. The previous app instance is killed (with its PID file)
7. The `current` symlink is redirected to the new directory
8. The `start.st` script is run against he newly built image and the PID of the
   process is stored in `app.pid`
9. Tadada, you've deployed a new version of your app.
